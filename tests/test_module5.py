...
def test_app_job_template_module5():
    ...
    assert 'show_job:job' in template_functions('job',
                                                'show_job'), 'Have you call the `show_job` macro in the `job.html` file?'
...
def test_app_job_route_module5():
    ...
    assert return_values['value/args/s'] == 'job.html' and return_values[
...
def test_app_job_route_parameter_module5():
    assert 'job' in dir(app), 'Have you created the `job` function?'
    assert 'job:job_id:job:id' in template_functions('_macros',
                                                     'url_for'), 'Looks like the job title link `href` is incorrect.'
