...
def test_show_job_macro_definition_module4():
    ...
    assert 'show_job:job' in template_macros(
        '_macros'), 'Have you created the `show_job` macro and added the correct parameter?'
...
def test_show_jobs_macro_definition_module4():
    ...
    assert 'show_jobs:jobs' in template_macros(
        '_macros'), 'Have you created the `show_jobs` macro and added the correct parameter?'
...
def test_show_jobs_macro_for_loop_module4():
    ...
    assert len(
        div) == 1, 'Has a `<div>` with classes of `columns` and `is-multiline` been added to the `show_jobs` macro?'
...
def test_show_jobs_macro_for_loop_body_module4():
    ...
    assert len(
        div) == 1, 'Has a `<div>` with classes of `column` and `is-half` been added to the `show_jobs` macro `for` loop body?'
...
def test_import_macros_module4():
    ...
    assert '_macros.html:show_job:show_jobs:True' == template_import(
        'layout'), 'Have you imported `_macros.html` in `layout.html`?'
...
def test_display_all_jobs_module4():
    assert template_exists('index'), 'The `index.html` template does not exist in the `templates` folder.'
    assert 'show_jobs:jobs' in template_functions('index',
                                                  'show_jobs'), 'Have you call the `show_jobs` macro in the `index.html` file?'
...
def test_app_jobs_route_jobs_module4():
    ...
    assert 'render_template:index.html:jobs:jobs' in get_functions(
        app.jobs), 'Have you added `jobs` to the `render_template` call.'
